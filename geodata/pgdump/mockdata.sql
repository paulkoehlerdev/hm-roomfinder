...
SELECT 2 as level_id, concat('F1.', substring(name FROM 4)) as name, attr, geom FROM room WHERE level_id = 1;

INSERT INTO room (level_id, name, attr, geom)
SELECT 3 as level_id, concat('F2.', substring(name FROM 4)) as name, attr, geom FROM room WHERE level_id = 1;